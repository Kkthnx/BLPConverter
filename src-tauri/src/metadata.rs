...
pub fn file_kind_from_path(path: &Path) -> Option<AssetKind> {
    match path
        .extension()
        .and_then(|e| e.to_str())
        ...
        .as_deref()
    {
        Some("png") => Some(AssetKind::Png),
        Some("blp") => Some(AssetKind::Blp),
        _ => None,
    }
}
    ...
    let kind = match extension.as_str() {
        ...
        _ => {
            return FileMetadata {
                ...
                mipmap_count: 0,
                kind: AssetKind::Blp,
                error: Some("Unsupported file extension".into()),
            };
        }
    };
